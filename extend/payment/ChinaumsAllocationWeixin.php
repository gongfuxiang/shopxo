 ...
 * @desc    description
 */
class ChinaumsAllocationWeixin
{
    // 插件配置参数
    private $config;
    ...
    public function Config()
    {
        // 基础信息
        $base = [
            'name'          => '银联分账-微信小程序',  // 插件名称
            'version'       => '1.0.0',  // 插件版本
            'apply_version' => '不限',  // 适用系统版本描述
            'apply_terminal'=> ['weixin'], // 适用终端 默认全部 ['pc', 'h5', 'app', 'alipay', 'weixin', 'baidu']
            'desc'          => '适用微信小程序，分账模式支付方式。 <a href="https://www.chinaums.com/" target="_blank">立即申请</a>',  // 插件描述（支持html）
            'author'        => 'Devil',  // 开发者
    ...
    public function Pay($params = [])
    {
        ...
        // 是否跳转小程序收银台
        $is_to_mini_cashier = (isset($this->config['is_to_mini_cashier']) && $this->config['is_to_mini_cashier'] == 1) ? '-mini' : '';
        return PaymentHandleService::Pay('weixin'.$is_to_mini_cashier, $params);
    }
