...
Page({
  ...
  onLoad(params) {
    // 启动参数处理
    params = app.launch_params_handle(params); // 参数赋值,初始化
    //params['goods_id']=2;
  ...
  onShow() {
    tt.setNavigationBarTitle({
      title: this.data.goods == null ? app.data.common_pages_title.goods_detail : this.data.goods.title
    });
  },
  ...
  init() {
    ...
    if ((this.data.params.goods_id || null) == null) {
      ...
      tt.request({
        ...
        method: "POST",
        data: {
          goods_id: this.data.params.goods_id
        },
        dataType: "json",
        header: {
  ...
  goods_specifications_event(e) {
    ...
    var temp_data = this.data.goods_specifications_choose;
    var temp_images = this.data.goods_spec_base_images; // 不能选择和禁止选择跳过
    ...
    if ((temp_data[key]['value'][keys]['is_dont'] || null) == null && (temp_data[key]['value'][keys]['is_disabled'] || null) == null) {
      ...
      this.setData({
        ...
        temp_buy_number: this.data.goods.buy_min_number || 1
      }); // 不能选择规格处理
  ...
  plugins_limitedtimediscount_countdown() {
    ...
    clearInterval(this.data.plugins_limitedtimediscount_timers); // 定时参数
